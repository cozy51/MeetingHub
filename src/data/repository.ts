...
export const repository = {
  ...
  parseImport(raw:string):{meetings:Meeting[];categories:Category[];holidays:Holiday[]} { const data=JSON.parse(raw); if(!Array.isArray(data.meetings)) throw new Error("meetings がありません"); return {meetings:data.meetings,categories:Array.isArray(data.categories)?data.categories:defaultCategories,holidays:Array.isArray(data.holidays)?data.holidays:[]}; }
};
